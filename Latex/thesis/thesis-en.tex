...
\tableofcontents
\thispagestyle{empty}
% IF YOU HAVE EVEN QUANTITY OD PAGES OF TOC, THEN REMOVE IT OR ADD \null\newpage FOR DOUBLE BLANK PAGE BEFORE INTRODUCTION
...
\begin{enumerate}
    \item How do RF-DETR, Swin Transformer detection, YOLOv8, and YOLO26 compare on the accuracy--latency--size trade-off when evaluated under the same COCO protocol?
...
\section{Object detection architectures considered in this work}
...
\subsection{YOLO-style one-stage detectors}
YOLO introduced a one-stage formulation in which object classes and bounding boxes are predicted directly from an image in a single forward pass \cite{Redmon2016YOLO}. This idea is attractive for real-time deployment because it avoids the separate region-proposal stage used by two-stage detectors such as Faster R-CNN \cite{Ren2015FasterRCNN}. Later YOLO implementations improved the training recipe, feature aggregation, anchor handling, loss functions, export tooling, and hardware deployment support. In this thesis, YOLOv8 is used as a mature Ultralytics baseline, while YOLO26 is included as a newer comparison point intended to represent the current direction of deployment-oriented YOLO models \cite{UltralyticsYOLOv8, UltralyticsYOLO26}.

\subsection{Detection transformers and RF-DETR}
DETR-style models formulate object detection as a set prediction problem, replacing many hand-designed post-processing components with transformer attention and learned object queries. This family is conceptually important because it connects object detection with the broader success of transformer architectures in vision and language \cite{Vaswani2017Attention, Dosovitskiy2020ViT}. RF-DETR is especially relevant for this thesis because it targets the real-time part of the design space: instead of only maximising accuracy, it searches for accuracy--latency Pareto-efficient detection transformer variants and provides COCO-pretrained checkpoints suitable for practical evaluation \cite{Robinson2025RFDETR}. It therefore serves as the main transformer detector baseline in the experimental part.

\subsection{Swin Transformer detectors}
Swin Transformer addresses a key limitation of plain ViTs for dense vision tasks: global attention over all image patches becomes expensive at high resolution. Swin uses a hierarchical representation and shifted local windows, which reduces attention cost while still allowing information to move across neighbouring windows \cite{Liu2021Swin}. This makes it suitable for object detection and segmentation, where multi-scale spatial features are important. In this thesis, a Swin-T Mask R-CNN detector is treated as the hierarchical vision-transformer representative. It is not expected to be the fastest model, but it provides a useful contrast to both RF-DETR and YOLO: it combines transformer-style feature extraction with a conventional detection framework.


\section{Token reduction for vision transformers}

Token reduction is an efficiency direction specific to transformer-style models. In a vision transformer, an image is represented as a sequence of patch tokens. Since self-attention compares tokens with one another, reducing the number of tokens can lower both computation and memory use, especially in deeper layers where the representation is already semantically richer \cite{Vaswani2017Attention, Dosovitskiy2020ViT}. This makes token reduction complementary to quantisation and pruning: quantisation reduces numerical precision, pruning removes model structures or parameters, while token reduction reduces the amount of intermediate representation processed during inference.

\subsection{Token pruning and token merging}
...
\section{Research objective and validation logic}
...
\begin{itemize}
    \item RF-DETR Medium FP32 and FP16 evaluation on the full COCO 2017 validation set,
    \item YOLOv8 and YOLO26 FP32/FP16 inference on the COCO validation split,
    \item YOLOv8 and YOLO26 INT8 post-training quantisation through TensorRT engines,
    \item telemetry collection for latency, throughput, power, energy per image, GPU memory, utilisation, and temperature.
\end{itemize}
...
\section{Dataset preparation and calibration protocol}
...
\begin{itemize}
    \item model family and checkpoint identifier,
    \item precision variant (FP32, FP16, INT8 PTQ),
    \item runtime backend (PyTorch, TensorRT, or pending backend),
    \item number of evaluated images,
    \item COCO mAP metrics,
    \item latency, throughput, GPU memory, power, and energy fields when available,
    \item path to the model artifact or exported engine.
\end{itemize}
...
\section{Solution architecture and implementation environment}
...
\section{Runtime and export backends}
...
CNN/YOLO baseline & YOLOv8s/m and YOLO26n/s/m & Dense convolutional one-stage detector line with widely used COCO checkpoints. YOLOv8 provides a historical Ultralytics baseline, while YOLO26 adds a newer NMS-free deployment-oriented comparison point \cite{UltralyticsYOLOv8, UltralyticsYOLO26}. \\
\hline
\end{tabular}
...
\section{Completed experiment scope}
...
The reported latency is median per-image latency at batch size 1. For the YOLO INT8 rows, the runtime is TensorRT and the engines were produced through post-training quantisation with a COCO calibration subset. The raw result artifacts are retained under the project \texttt{runs/} directory together with exported model artifacts and plot files used in the presentation.

\section{Data used in the preliminary experiments}

The preliminary experiments use the COCO 2017 validation split. The full validation set contains 5000 images and an annotation file in JSON format. Each image entry contains fields such as image identifier, file name, width, and height. Each object annotation contains an image identifier, category identifier, bounding box, segmentation metadata, object area, and an \texttt{iscrowd} flag. The most important numeric fields for this thesis are the bounding boxes and category identifiers, because they are used to compute COCO mAP and AP@0.5.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.88\linewidth]{../DiplomaSeminar2_Presentation2/assets/ap_latency_scatter.pdf}
    \caption{Accuracy--latency overview of the completed preliminary measurements. Accuracy values are reported against the COCO 2017 validation target; the separate 500-image subset is used for INT8 calibration and smoke tests, not as the main accuracy benchmark.}
    \label{fig:prelim_ap_latency_scatter}
\end{figure}

Figure~\ref{fig:prelim_ap_latency_scatter} summarises the current state of the experiment matrix. The RF-DETR points occupy the high-accuracy but higher-latency region. YOLO and YOLO26 points occupy the lower-latency region, especially after TensorRT INT8 export. The figure also shows why a single metric is insufficient: RF-DETR is currently the strongest measured detector in accuracy, while several YOLO-family INT8 engines are much faster at the cost of lower mAP.

\section{RF-DETR baseline}
...
\hline
\end{tabular}
\caption{Preliminary RF-DETR Medium results on the full COCO 2017 validation set.}
\label{tab:prelim_rfdetr}
\end{table}
...
\section{YOLO and YOLO26 precision comparison}
...
\hline
\end{tabular}
\caption{Preliminary YOLO and YOLO26 results on the COCO 2017 validation target.}
\label{tab:prelim_yolo_precision}
\end{table}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{../DiplomaSeminar2_Presentation2/assets/yolo_latency_by_precision.pdf}
    \caption{Median batch-size-1 latency for YOLO-family models under FP32, FP16, and TensorRT INT8.}
    \label{fig:yolo_latency_by_precision}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.86\linewidth]{../DiplomaSeminar2_Presentation2/assets/yolo_ap_by_precision.pdf}
    \caption{Detection quality for YOLO-family models under precision changes on the COCO 2017 validation target.}
    \label{fig:yolo_ap_by_precision}
\end{figure}
...
\hline
\end{tabular}
\caption{Selected telemetry values from the preliminary runs. Energy estimates are approximate and depend on the sampled benchmark window.}
\label{tab:prelim_energy}
\end{table}
