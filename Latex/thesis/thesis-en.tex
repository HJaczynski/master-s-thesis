...
\title
\end{center}
Deep learning models achieve high accuracy in computer vision, but their computational and memory demands often prevent direct deployment on resource-constrained edge devices such as cameras, robots, drones, and mobile platforms. This thesis examines how to make modern object detection models suitable for real-time on-device inference by reducing their computational cost while maintaining detection quality. The work reviews the main architectural families used in vision (convolutional networks and transformer-based models) and then focuses on practical efficiency methods applicable to edge deployment. In particular, it studies model compression and acceleration techniques, including quantisation (post-training and quantisation-aware training), pruning (structured and unstructured), and knowledge distillation, as well as architecture-level approaches such as hardware-aware neural architecture search. The proposed evaluation emphasises realistic deployment conditions and compares methods using consistent metrics, including detection accuracy, inference latency, and model size. Where feasible, it also considers energy-related aspects. The overall goal is to identify which techniques, or combinations of techniques, provide the most favourable accuracy--latency--size trade-off for real-time object detection on constrained hardware and how strongly the results depend on the target device and inference runtime.

\noindent \textbf{Keywords:} Model compression, efficiency optimisation, deep learning, neural networks, quantisation, pruning, knowledge distillation, hardware-aware neural architecture search, edge device deployment
\end{abstract}
}
...
This thesis focuses on real-time object detection on resource-limited devices. The goal is to investigate and compare methods that reduce the computational and memory requirements of object detection models, enabling them to run efficiently on edge hardware without compromising detection quality. The work will review modern approaches used for on-device deployment, implement selected solutions, and evaluate them using consistent and practical metrics such as detection accuracy, inference latency, model size, and (where possible) energy-related measurements.

The study is guided by the following research questions:

\begin{enumerate}
    \item Which model architectures are the most appropriate for deployment on resource-limited devices when considering the trade-off between accuracy and computational cost?
    \item Which methods for making models smaller and faster, or combinations of such methods, provide the best trade-off between accuracy, inference speed, model size, and energy usage on edge devices?
    \item How do post-training quantisation and quantisation-aware training differ in their impact on object detection accuracy and inference latency?
    \item Under what conditions can training a smaller model to mimic a larger one provide better results than relying only on direct training or model simplification?
    \item To what extent do the final results depend on the target hardware and the software stack used for deployment?
\end{enumerate}
